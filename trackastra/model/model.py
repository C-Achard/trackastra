...
class TrackingTransformer(torch.nn.Module):
    def __init__(
        ...
        self.feature_proj = nn.Linear(feat_dim, d_model) if feat_dim > 0 else nn.Identity()
        self.feature_proj_dropout = nn.Dropout(input_proj_dropout) if feat_dim > 0 else nn.Identity()
        self.feature_attention = nn.MultiheadAttention(
            embed_dim=d_model, num_heads=4, dropout=input_proj_dropout
        )
        self.proj = nn.Linear(
    ...
    def forward(self, coords, features=None, padding_mask=None):
        ...
        with torch.amp.autocast(enabled=False, device_type=features.device.type):
            if features is None or features.numel() == 0:
                ...
                features = self.feature_proj_dropout(features)
                features = self.feature_attention(
                    features.transpose(0, 1), features.transpose(0, 1), features.transpose(0, 1)
                )[0].transpose(0, 1)
