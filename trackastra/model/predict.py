...
def predict(batch: list[dict], model: TrackingTransformer) -> np.ndarray:
    ...
    if padded_batch["features"] is not None:
        ...
        feats = None
    if padded_batch["pretrained_features"] is not None:
        pretrained_feats = padded_batch["pretrained_features"]
    else:
    ...
    if pretrained_feats is not None:
        pretrained_feats = pretrained_feats.to(device)
...
def predict_windows(
    ...
    for t in progbar_class(
        ...
        if pred_func_override is not None:
            A_batch = pred_func_override(batch)
        else:
